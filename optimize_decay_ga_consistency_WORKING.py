...
Optimizes for CONSISTENT CALIBRATION (low Brier score variance) across rolling VALIDATION windows.
OOS data is NEVER used during optimization - only for final blind evaluation.

Since validation data has no market odds, we optimize for:
- Low mean Brier score (good calibration)
- Low std Brier score (consistency across time periods)
- Sharpe-like ratio: -mean_brier / std_brier (negative because lower Brier is better)
"""
...
def run_elo_with_params(df, k=170, base_elo=1500, denominator=400, use_mov=True,
    ...
    df["win_prob"] = 1 / (1 + 10 ** ((df["opp_precomp_elo"] - df["precomp_elo"]) / denominator))
    
    return df, ratings, last_fight_date

def calculate_brier_score(df_window):
    """Calculate Brier score for a dataframe window."""
    # Filter for fighters with postcomp_boutcount > 1
    df_window['postcomp_boutcount_num'] = pd.to_numeric(df_window['postcomp_boutcount'], errors='coerce')
    df_eval = df_window[df_window['postcomp_boutcount_num'] > 1].copy()
    
    if len(df_eval) < 5:  # Minimum fights
        return None, 0
    
    # Brier score = mean((predicted_prob - actual_result)^2)
    brier = np.mean((df_eval['win_prob'] - df_eval['result']) ** 2)
    
    return brier, len(df_eval)

def calculate_roi_for_window(df_window, market_odds_col='avg_odds'):
    """Calculate ROI for a dataframe window (for OOS evaluation only)."""
    df_window['postcomp_boutcount_num'] = pd.to_numeric(df_window['postcomp_boutcount'], errors='coerce')
    df_eval = df_window[df_window['postcomp_boutcount_num'] > 1].copy()
...
def evaluate_consistency(individual, full_df):
    """
    Evaluate consistency using Brier score across rolling VALIDATION windows.
    NEVER touches OOS data - maintains experimental integrity.
    
    Optimizes for:
    - Low mean Brier score (good calibration)
    - Low std Brier score (consistency)
    - Fitness = -mean_brier / std_brier (Sharpe-like, negative because lower is better)
    ...
    try:
        ...
        for i in range(num_windows):
            ...
            brier, n_fights = calculate_brier_score(window_df)
            if brier is not None and n_fights >= 5:  # Minimum 5 fights per window
                brier_scores.append(brier)
        
        if len(brier_scores) < 2:  # Need at least 2 windows for std
            return (-1e10,)
        ...
        if std_brier < 0.001:  # Avoid division by near-zero
            # If very consistent, reward low mean
            fitness = -mean_brier * 100  # Negative because lower Brier is better
        else:
            # Sharpe-like ratio: reward low mean and low variance
            # Negative because we want LOW Brier (unlike ROI where we want HIGH)
            fitness = -mean_brier / std_brier
        
        # Bonus for very low Brier scores
        if mean_brier < 0.20:
            fitness += 5
...
def main():
    ...
    print("(Optimizing for Calibration Consistency via Brier Score)")
    print("=" * 80)
    print()
    print("Objective: Minimize Brier score variance across validation windows")
    print("Method: Rolling window Brier score analysis on VALIDATION data only")
    print("OOS: Completely untouched during optimization - used only for final blind test")
    print()
    print("Why Brier Score?")
    print("- Validation data has no market odds, so can't calculate ROI")
    print("- Brier score measures calibration quality")
    print("- Consistent calibration should generalize to OOS")
    print()
    ...
    print("Optimizing for calibration consistency...")
    print()
    ...
    print("=" * 80)
    print()
    print(f"Best parameters (by calibration consistency on validation):")
    ...
    print(f"  Calibration fitness: {best.fitness.values[0]:.4f}")
    print()
    ...
    val_brier, val_fights = calculate_brier_score(val_result)
    val_result['postcomp_boutcount_num'] = pd.to_numeric(val_result['postcomp_boutcount'], errors='coerce')
    val_acc = val_result[val_result['postcomp_boutcount_num'] > 1]['result'].mean() * 100 if len(val_result[val_result['postcomp_boutcount_num'] > 1]) > 0 else None
    
    print(f"\nValidation Performance (Full Year):")
    print(f"  Brier Score: {val_brier:.4f}" if val_brier else "  Brier Score: N/A")
    ...
    try:
        ...
        oos_df['postcomp_boutcount'] = '2'  # Assume > 1 for OOS evaluation
        ...
        oos_brier, oos_brier_fights = calculate_brier_score(oos_only)
        oos_roi, oos_roi_fights = calculate_roi_for_window(oos_only)
        oos_only['postcomp_boutcount_num'] = pd.to_numeric(oos_only['postcomp_boutcount'], errors='coerce')
        oos_acc = oos_only[oos_only['postcomp_boutcount_num'] > 1]['result'].mean() * 100 if len(oos_only[oos_only['postcomp_boutcount_num'] > 1]) > 0 else None
        
        print(f"\nOOS Performance (BLIND TEST - Never Used in Optimization):")
        print(f"  Brier Score: {oos_brier:.4f}" if oos_brier else "  Brier Score: N/A")
        print(f"  ROI: {oos_roi:.2f}%" if oos_roi else "  ROI: N/A")
        print(f"  Accuracy: {oos_acc:.2f}%" if oos_acc else "  Accuracy: N/A")
        print(f"  Fights: {oos_roi_fights}")
        print()
        
        if val_brier and oos_brier:
            gap = abs(val_brier - oos_brier)
            print(f"Validation-OOS Brier Gap: {gap:.4f}")
            print()
            
            if gap < 0.02:
                print("✓✓ EXCELLENT CONSISTENCY - Brier gap < 0.02")
                print("   Calibration shows strong generalization!")
            elif gap < 0.05:
                print("✓ GOOD CONSISTENCY - Brier gap < 0.05")
                print("   Calibration shows reasonable generalization.")
            else:
        ...
        import traceback
        traceback.print_exc()
        oos_brier, oos_roi, oos_acc, oos_roi_fights = None, None, None, 0
    
    # Save results
    results = {
        'approach': 'consistency_calibration_brier',
        'optimization_target': 'brier_score_consistency_on_validation_windows',
        ...
        'calibration_fitness': float(best.fitness.values[0]) if not np.isinf(best.fitness.values[0]) else None,
        'validation_performance': {
            'brier_score': float(val_brier) if val_brier else None,
            'accuracy': float(val_acc) if val_acc else None,
            'n_fights': int(val_fights)
        },
        'oos_performance': {
            'brier_score': float(oos_brier) if oos_brier else None,
            'roi': float(oos_roi) if oos_roi else None,
            'accuracy': float(oos_acc) if oos_acc else None,
            'n_fights': int(oos_roi_fights),
            'note': 'Blind test - never used during optimization'
        },
        'val_oos_brier_gap': float(abs(val_brier - oos_brier)) if (val_brier and oos_brier) else None
    }
