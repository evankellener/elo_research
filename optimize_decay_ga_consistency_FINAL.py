...
def calculate_roi_for_window(df_window, market_odds_col='avg_odds'):
    ...
    df_eval = df_window[df_window['postcomp_boutcount'] > 1].copy()
...
def main():
    ...
    val_acc = val_result[val_result['postcomp_boutcount'] > 1]['result'].mean() * 100 if len(val_result[val_result['postcomp_boutcount'] > 1]) > 0 else None
    ...
    try:
        ...
        # Add boutcount columns for consistency
        oos_df['postcomp_boutcount'] = oos_df.get('boutcount', 2)  # Assume > 1 for OOS
        ...
        oos_acc = oos_only[oos_only['postcomp_boutcount'] > 1]['result'].mean() * 100 if len(oos_only[oos_only['postcomp_boutcount'] > 1]) > 0 else None
