...
class TestGASearchParamsROI(unittest.TestCase):
    ...
    def test_lookback_days_parameter(self):
        """Test that lookback_days parameter is accepted and works correctly"""
        df = pd.read_csv(self.data_file, low_memory=False)
        odds_df = pd.read_csv(self.odds_file, low_memory=False)
        ...
        # Test with different lookback_days values (365, 180, 30)
        for lookback in [365, 180, 30]:
            best_params, best_roi = ga_search_params_roi(
                df,
                odds_df,
                population_size=3,
...
class TestMainBlockModeRouting(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.parser.add_argument("--lookback-days", type=int, default=365,
    ...
    def test_lookback_days_default_is_365(self):
        """Test that the default lookback_days is 365"""
        args = self.parser.parse_args([])
        self.assertEqual(args.lookback_days, 365, "Default lookback_days should be 365")
    
    def test_lookback_days_365_explicit(self):
        """Test that --lookback-days 365 is correctly parsed"""
        args = self.parser.parse_args(["--lookback-days", "365"])
        self.assertEqual(args.lookback_days, 365)
    
    def test_lookback_days_180_explicit(self):
        """Test that --lookback-days 180 is correctly parsed (past 6 months)"""
        args = self.parser.parse_args(["--lookback-days", "180"])
        self.assertEqual(args.lookback_days, 180)
    
    def test_lookback_days_30_explicit(self):
        """Test that --lookback-days 30 is correctly parsed (past month)"""
        args = self.parser.parse_args(["--lookback-days", "30"])
        self.assertEqual(args.lookback_days, 30)
    
    def test_lookback_days_with_mode(self):
        """Test that --lookback-days and --mode can be used together"""
